# Activities  
**Author:** Joe Goode  
**Courses:** C# Programming, .NET (Practice Activities)

## Overview
This folder contains all of the smaller practice activities completed throughout the C# and .NET courses.

## What’s Included
- Small C# console programs
- A simple project file and source code  

## Current Activities
**Course: C#**
- Activity 01 – Average Calculator  
- Activity 02 – Temperature Converter  
- Activity 03 – Exam Score Categorizer
- Activity 04 – Zoo Animals
- Activity 05 – Email Notifications

**Course: .NET**
- Activity 06 – String Manipulation  
- Activity 07 – Date/Time App
- Activity 08 – Collection Manipulation
- Activity 09 - File I/O
- Activity 10 - Exception Handling